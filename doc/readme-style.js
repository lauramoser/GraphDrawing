;(async function run() {
  ...
  function findRoot() {
    ...
    const hrefAttr = cssElem.getAttribute('href')
    if (hrefAttr.indexOf('/doc/') !== -1) {
      return hrefAttr.replace(/^(.*\/)(doc\/.*)$/, '$1')
    }
    const docRel = hrefAttr.replace(/^((?:\.\.\/)*).*$/, '$1')
  ...
  function getLink(root, url) {
    return location.pathname === url ? '.' : `${root}${url}`
  }
  ...
  function createHeader(title, simpleHeader, hideToggleSidebarButton) {
    ...
    <div class="header-group-right">
      <div class="status-bar-version">Version 2.6.0.4 Complete </div>
      ...
      <div class='center'>
        <div class="title">
          ...
          <a href="${getLink(
            root,
            'README.html'
          )}" title='yFiles for HTML home'><div class="logo"></div></a>
        </div>
      </div>
      <div class='right'>
        ${groupRight}
  ...
  function createDetailedFooter() {
    ...
    <div class="footer-block">
      ...
      <br><br> <span class="copyright">COPYRIGHT &#x00A9; 2024 yWorks</span><br> <a
    ...
    container.innerHTML = `<div class="yfiles-ui-banner">
      ...
      <div class="content collapsed">
        ...
        <div class="yfiles-ui-not-running" style="display: none">
          ...
          <p>yFiles Dev Suite is not running.</p> <span><b>Start</b> it in <b>your terminal</b> with </span><code>&gt; npx yfiles-dev-suite</code>
        </div>
...
<div><b>Start</b> it now:<pre>&gt; npx yfiles-dev-suite</pre></div>
          Features:
          <ul>
...
`
  ...
  function checkDemoServer() {
    ...
    fetch(demoServerUrl)
      .then(resp => {
        if (resp.headers.get('x-yfiles-for-html-dev-server')) {
